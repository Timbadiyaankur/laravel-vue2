        ...
        <b-col sm="12" md="4">
          <validation-observer ref="productForm" v-slot="{ handleSubmit, invalid }">
            <b-form @submit.prevent="handleSubmit(onSubmit)">
              ...
              <b-row>
                <b-col>
                  <b-button type="submit" variant="primary" :disabled="invalid">Save</b-button>
                </b-col>
              </b-row>
            </b-form>
          </validation-observer>
...
export default {
  ...
  async mounted() {
    await this.findByProductId(this.$route.params.id)
  },
  computed: {
    ...mapState('product', { form: 'product' }),
  },
  methods: {
    ...mapActions('product', { updateProduct: 'update', findByProductId: 'findById' }),
    ...mapMutations('product', { resetForm: 'emptyProduct' }),
    onSubmit() {
      this.$apiHandler.apiResponseWrapper(async () => {
        const res = await this.updateProduct({ id: this.$route.params.id, data: this.form })
        this.resetForm()
        this.$router.push({
          name: 'product-list',
        })
